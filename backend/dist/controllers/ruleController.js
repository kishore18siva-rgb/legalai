...
exports.rejectRule = exports.approveRule = exports.updateRule = exports.createRule = exports.getRuleById = exports.listRules = void 0;
...
const getRuleById = async (req, res) => {
    try {
        ...
        if (!rule)
            return res.status(404).json({ error: 'Legal rule not found.' });
...
const createRule = async (req, res) => {
    try {
        const { ruleCode, ruleNumber, subRule, clause, schedule, title, requirementText, applicabilityCondition, validationType, parameters, capabilityClass, sourcePage, sourceText, } = req.body;
...
const rejectRule = async (req, res) => {
    try {
        const { id } = req.params;
        const rule = await prisma.legalRule.update({
            where: { id },
