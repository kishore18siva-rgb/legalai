...
class OcrService {
     ...
     */
    async processImage(imagePath, imageType = 'FRONT', originalName) {
        ...
        try {
            if (fs_1.default.existsSync(imagePath)) {
                const fileBuf = fs_1.default.readFileSync(imagePath);
        ...
        try {
            ...
            const sharpImg = (0, sharp_1.default)(imagePath);
            ...
            const boundingBoxes = validWords.map((w) => ({
                x0: w.bbox.x0,
                y0: w.bbox.y0,
                x1: w.bbox.x1,
                y1: w.bbox.y1,
